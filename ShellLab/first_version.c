...
void f_fg(struct cmdline_tokens *tok);
void f_bg(struct cmdline_tokens *tok);
...
int builtin_command(struct cmdline_tokens *tok)
{
    if(tok->builtins == BUILTIN_QUIT)
        exit(0);
    if(tok->builtins == BUILTIN_JOBS && !tok->outfile)
    {
    ...
    if(tok->builtins == BUILTIN_JOBS && tok->outfile)
    {
        int fd = open(tok->outfile, O_RDWR, 0);
    ...
    {
        //printf("flag1\n");
        int fd = open(tok->infile, O_RDWR, 0);
        char c;
        while (read(fd, &c, 1) != 0)
        {
    ...
    if(tok->infile && tok->outfile)
    {
        int fdin = open(tok->infile, O_RDWR, 0);
        int fdout = open(tok->outfile, O_RDWR, 0);
        char c;
...
eval(char *cmdline) 
{
    ...
    sigemptyset(&mask_one);//mask_one is empty
    ...
    if (tok.argv[0] == NULL) /* ignore empty lines */
        return;
    sigprocmask(SIG_BLOCK, &mask_all, &prev_one);//block EVERY mask!!! include SIGINT!!!!
    if(!builtin_command(&tok))//not built-in
    {
        //printf("must\f");
        if((pid = fork()) == 0)//child 
        {
            //printf("hello from child\n");
            int _setpgid_flag = setpgid(0, 0);
            if(_setpgid_flag == -1)
            {
